<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Modifier le règlement {{ $payment->payment_number }}</h1>
        <a href="{{ route('supplier-payments.show', $payment->id) }}" class="text-sm text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Retour au détail</a>
    </div>
    ...
    <div class="flux-card">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div>
                <flux:input label="Fournisseur" :value="$payment->supplier?->name" disabled />
            </div>
            <div>
                <flux:input wire:model="payment_date" type="date" label="Date de paiement" required />
            </div>
            <div>
                <flux:input label="Moyen de paiement" :value="$payment->paymentMethod?->name" disabled />
            </div>
            <div>
                <flux:input label="Journal" :value="$payment->journal ? $payment->journal->code.' — '.$payment->journal->name : '—'" disabled />
            </div>
            <div>
                <flux:input label="Compte de destination" :value="$payment->destinationAccount ? $payment->destinationAccount->code.' — '.$payment->destinationAccount->name : '—'" disabled />
            </div>
            <div>
    ...
    <div class="flux-card">
        ...
        @if (count($this->allocations) === 0)
            <p class="text-sm text-gray-500 dark:text-gray-400">Aucune facture ouverte pour ce fournisseur. Le règlement sera enregistré comme avance (non imputé).</p>
        @else
    ...
    <div class="flex justify-end gap-3">
        ...
        <button wire:click="save(true)" wire:confirm="Voulez-vous enregistrer et comptabiliser ce règlement ? Une écriture comptable sera générée." class="flux-btn-green">Enregistrer et comptabiliser</button>
    </div>
</div>
