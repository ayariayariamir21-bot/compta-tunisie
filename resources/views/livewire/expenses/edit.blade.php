<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Modifier la dépense {{ $expense->expense_number }}</h1>
        <a href="{{ route('expenses.show', $expense) }}" class="text-sm text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Retour au détail</a>
    </div>
    ...
    <div class="flux-card">
        ...
        <div class="mt-4 flex flex-col items-end gap-1 border-t border-gray-200 pt-4 dark:border-gray-700">
            ...
            <p class="text-base text-gray-500 dark:text-gray-400">
                Total TTC : <span class="font-bold text-gray-900 dark:text-white">{{ number_format((float) $this->totals['total'], 3, '.', '') }} {{ $expense->currency }}</span>
            </p>
        </div>
    </div>

    <div class="flex justify-end gap-3">
        <button wire:click="save(false)" class="flux-btn-primary">Enregistrer les modifications</button>
        <button wire:click="save(true)" wire:confirm="Voulez-vous enregistrer et comptabiliser cette dépense ? Une écriture comptable sera générée." class="flux-btn-green">Enregistrer et comptabiliser</button>
    </div>
</div>
