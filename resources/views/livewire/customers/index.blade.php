        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                    @if ($currentCompany)
                        Clients de « {{ $currentCompany->name }} ».
                    @else
                        Sélectionnez une société pour voir les clients.
                    @endif
                </p>
            </div>

            @if ($currentCompany)
    @can('create', [App\Models\Customer::class, $currentCompany])
                        ...
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @foreach ($customers as $customer)
                                <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-800">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-neutral-500">
                                        {{ $customer->account ? $customer->account->code : '—' }}
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-3 text-sm">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-right text-sm">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @if (Auth::user()->can('activate', $customer) || Auth::user()->can('deactivate', $customer))
                                                <button
                                                    wire:click="toggleActive({{ $customer->id }})"
                                                    wire:confirm="{{ $customer->is_active ? 'Désactiver ce client ?' : 'Activer ce client ?' }}"
                                                    class="rounded-md px-2 py-1 text-sm transition hover:bg-neutral-100 dark:hover:bg-neutral-800 {{ $customer->is_active ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400' }}"
                                                >
                                            ...
                                            @if (Auth::user()->can('delete', $customer))
                                                <button
                                                    ...
                                                    wire:confirm="Supprimer le client « {{ $customer->name }} » ?"
