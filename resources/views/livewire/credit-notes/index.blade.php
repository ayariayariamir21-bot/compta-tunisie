            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <div>
                    <flux:select wire:model.live="filterStatus" label="Statut" placeholder="Tous">
                        ...
                        @foreach($creditNoteStatuses as $status)
                ...
                <div>
                    <flux:select wire:model.live="filterInvoiceId" label="Facture source" placeholder="Toutes">
            ...
            <div class="flux-card">
                @if ($creditNotes->count() === 0)
                    <div class="py-12 text-center">
                        <flux:icon name="receipt-refund" class="mx-auto h-12 w-12 text-gray-400" />
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Aucun avoir trouvé.</p>
                    </div>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Facture source</th>
                                ...
                                @foreach($creditNotes as $creditNote)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium text-gray-900 dark:text-white">
                                            {{ number_format((float) $creditNote->total, 3, '.', '') }} {{ $creditNote->currency }}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                            <a href="{{ route('credit-notes.show', $creditNote->id) }}" class="text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Voir</a>
                                            @if ($creditNote->status->value === 'draft')
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <a href="{{ route('credit-notes.edit', $creditNote->id) }}" class="text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Modifier</a>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="post({{ $creditNote->id }})" wire:confirm="Voulez-vous vraiment comptabiliser cet avoir ?" class="text-green-600 hover:text-green-500 dark:text-green-400">Comptabiliser</button>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="cancel({{ $creditNote->id }})" wire:confirm="Voulez-vous vraiment annuler cet avoir ?" class="text-orange-600 hover:text-orange-500 dark:text-orange-400">Annuler</button>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="delete({{ $creditNote->id }})" wire:confirm="Voulez-vous vraiment supprimer cet avoir ? Cette action est irréversible." class="text-red-600 hover:text-red-500 dark:text-red-400">Supprimer</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
        ...
        @else
            <div class="flux-card">
                <div class="py-12 text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Veuillez sélectionner une société pour afficher les avoirs.</p>
                </div>
            </div>
        @endif
    </div>
