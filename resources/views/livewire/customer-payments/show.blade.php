    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Encaissement {{ $payment->payment_number }}</h1>
        ...
        <div class="flux-card grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                    @if ($payment->customer)
                        <a href="{{ route('customers.show', $payment->customer_id) }}" class="text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>{{ $payment->customer->name }}</a>
                    @else
                        —
                    @endif
                </p>
        ...
        <div class="flux-card">
            ...
            @if ($payment->allocations->count() === 0)
                <p class="text-sm italic text-gray-500 dark:text-gray-400">Aucune imputation — avance client.</p>
            @else
                                ...
                                <tr>
                                    <td class="px-4 py-3 text-sm">
                                        @if ($allocation->invoice)
                                            <a href="{{ route('invoices.show', $allocation->invoice_id) }}" class="font-medium text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>{{ $allocation->invoice->invoice_number }}</a>
                                        @else
                                            —
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $allocation->invoice?->status?->label() ?? '—' }}</td>
        ...
        <div class="flex justify-end gap-3">
            @if ($payment->status->value === 'draft')
                <a href="{{ route('customer-payments.edit', $payment->id) }}" class="flux-btn-secondary" wire:navigate>Modifier</a>
                <button wire:click="cancel" wire:confirm="Voulez-vous vraiment annuler cet encaissement ?" class="rounded-md border border-orange-300 px-4 py-2 text-sm font-medium text-orange-600 hover:bg-orange-50 dark:border-orange-700 dark:hover:bg-orange-900/20">Annuler l'encaissement</button>
                <button wire:click="delete" wire:confirm="Voulez-vous vraiment supprimer cet encaissement ? Cette action est irréversible." class="rounded-md border border-red-300 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:border-red-700 dark:hover:bg-red-900/20">Supprimer</button>
                <button wire:click="post" wire:confirm="Voulez-vous vraiment comptabiliser cet encaissement ? Cette action est irréversible." class="flux-btn-primary">Comptabiliser</button>
            @elseif ($payment->status->value === 'posted')
                <p class="text-sm italic text-gray-500 dark:text-gray-400">Encaissement comptabilisé — document immuable.</p>
            @else
                <p class="text-sm italic text-gray-500 dark:text-gray-400">Encaissement annulé.</p>
            @endif
        </div>
    </div>
