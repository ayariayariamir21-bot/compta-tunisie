    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Encaissements</h1>
    @can('create', [App\Models\CustomerPayment::class, $currentCompany])
            <a href="{{ route('customer-payments.create') }}" class="flux-btn-primary" wire:navigate>
                Nouvel encaissement
            </a>
    @endcan
        </div>
            ...
            <div class="flux-card">
                @if ($payments->count() === 0)
                    <div class="py-12 text-center">
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Aucun encaissement trouvé.</p>
                    </div>
                @else
                                ...
                                @foreach($payments as $payment)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                            <a href="{{ route('customer-payments.show', $payment->id) }}" class="text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Voir</a>
                                            @if ($payment->isDraft())
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <a href="{{ route('customer-payments.edit', $payment->id) }}" class="text-flux-600 hover:text-flux-500 dark:text-flux-400" wire:navigate>Modifier</a>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="post({{ $payment->id }})" wire:confirm="Voulez-vous vraiment comptabiliser cet encaissement ?" class="text-green-600 hover:text-green-500 dark:text-green-400">Comptabiliser</button>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="cancel({{ $payment->id }})" wire:confirm="Voulez-vous vraiment annuler cet encaissement ?" class="text-orange-600 hover:text-orange-500 dark:text-orange-400">Annuler</button>
                                                <span class="text-gray-300 dark:text-gray-600">|</span>
                                                <button wire:click="delete({{ $payment->id }})" wire:confirm="Voulez-vous vraiment supprimer cet encaissement ? Cette action est irréversible." class="text-red-600 hover:text-red-500 dark:text-red-400">Supprimer</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
        ...
        @else
            <div class="flux-card">
                <div class="py-12 text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Veuillez sélectionner une société pour afficher les encaissements.</p>
                </div>
            </div>
        @endif
    </div>
