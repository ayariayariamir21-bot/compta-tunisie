...
class CustomerPaymentService
{
     ...
     * The next number is derived from the highest existing number for the current
     * prefix (all statuses included, so numbers are never reused), then uniqueness
     * is verified before returning; the database unique constraint on
     * (company_id, payment_number) backstops concurrent generations.
     */
    public function generatePaymentNumber(int $companyId): string
    {
        ...
        $maxNumber = CustomerPayment::where('company_id', $companyId)
        ...
        for ($attempt = 0; $attempt < $maxTries; $attempt++) {
            ...
            if (! CustomerPayment::where('company_id', $companyId)->where('payment_number', $candidate)->exists()) {
                return $candidate;
            }
        }
     ...
     */
    public function createDraft(array $data): CustomerPayment
    {
        $companyId = (int) $data['company_id'];

        $customer = $this->validateCustomer((int) $data['customer_id'], $companyId);
        ...
        $this->validateAllocations($companyId, $customer->id, $amount, $allocationsData);

        return DB::transaction(function () use ($data, $companyId, $customer, $fiscalYear, $period, $method, $journal, $destination, $amount, $allocationsData): CustomerPayment {
            $payment = CustomerPayment::create([
                ...
                'amount' => $amount,
                'currency' => $data['currency'] ?? $customer->company->currency ?? 'TND',
            ...
            foreach ($allocationsData as $entry) {
                CustomerPaymentAllocation::create([
                    ...
                    'invoice_id' => (int) $entry['invoice_id'],
    ...
    /**
     * @param  array{payment_date?: string, amount?: string, reference?: string|null, notes?: string|null, allocations?: array<int, array{invoice_id: int, amount: string}>}  $data
     *
    ...
    /**
     * Create a DRAFT payment from a posted invoice.
     *
     * The customer and the invoice allocation are preselected with the invoice's
    ...
    public function createFromInvoice(
        ...
        return $this->createDraft([
            ...
            'allocations' => [
                ['invoice_id' => $invoice->id, 'amount' => $remaining],
            ],
        ]);
    }

    /**
     * Add or replace the allocation of a single invoice on a draft payment.
     * A non-positive amount removes the allocation.
     *
     * @throws \InvalidArgumentException
     */
    public function allocate(CustomerPayment $payment, int $invoiceId, string $amount): CustomerPayment
    {
        if ($payment->status !== CustomerPaymentStatus::DRAFT) {
            throw new \InvalidArgumentException('Seul un règlement en brouillon peut être imputé.');
        }

        return DB::transaction(function () use ($payment, $invoiceId, $amount): CustomerPayment {
            ...
            if ($invoice->customer_id !== $payment->customer_id) {
                throw new \InvalidArgumentException('La facture sélectionnée n\'appartient pas à ce client.');
            }
            ...
            foreach ($payment->allocations()->get() as $row) {
                ...
                $allocationsData[] = ['invoice_id' => (int) $row->invoice_id, 'amount' => (string) $row->amount];
            }

            if (bccomp($normalized, '0', 3) > 0) {
                $allocationsData[] = ['invoice_id' => $invoiceId, 'amount' => $normalized];
            }
            ...
            if (bccomp($normalized, '0', 3) <= 0) {
                CustomerPaymentAllocation::where('customer_payment_id', $payment->id)
    ...
    /**
     * Validate proposed allocations against company/customer/invoice rules,
     * per-invoice outstanding balances and the payment amount itself.
     *
     * @param  array<int, array{invoice_id: int, amount: string}>  $allocationsData
     *
     * @throws \InvalidArgumentException
     */
    public function validateAllocations(int $companyId, int $customerId, string $paymentAmount, array $allocationsData): void
    {
        if ($allocationsData === []) {
            return;
        }
        ...
        foreach ($allocationsData as $entry) {
            ...
            if ($invoice->customer_id !== $customerId) {
                throw new \InvalidArgumentException('La facture à imputer n\'appartient pas au client du règlement.');
            }
    ...
    /**
     * Posted credit-note impact on an invoice (cancelled and draft credit notes excluded).
     *
     * @return numeric-string
     */
    public function getPostedCreditNoteImpact(Invoice $invoice): string
    {
     ...
     * @return numeric-string
     */
    public function getPostedAllocatedAmount(Invoice $invoice): string
    {
    ...
    /**
     * Remaining amount to settle on an invoice:
     * total − posted credit-note impact − posted payment allocations.
     *
     * @return numeric-string
     */
    public function getInvoiceRemainingAmount(Invoice $invoice): string
    {
        $total = $this->toDecimal((string) $invoice->total);
        $credited = $this->getPostedCreditNoteImpact($invoice);
        $allocated = $this->getPostedAllocatedAmount($invoice);

        $remaining = bcsub(bcsub($total, $credited, 3), $allocated, 3);
     ...
     * posted invoices − posted credit notes − posted payment allocations.
     *
     * @return numeric-string
     */
    public function getCustomerOutstandingBalance(Customer $customer): string
    {
        $invoiceIds = Invoice::where('customer_id', $customer->id)
        ...
        $invoicesTotal = $this->decimalFromDb(
            Invoice::whereIn('id', $invoiceIds)->sum('total')
        );

        $creditsTotal = $this->decimalFromDb(
        ...
        $outstanding = bcsub(bcsub($invoicesTotal, $creditsTotal, 3), $paidTotal, 3);
    ...
    /**
     * Customer receivable account: customer.account_id first, then the company
     * default customer account from accounting settings.
     *
     * @throws \InvalidArgumentException
     */
    public function resolveReceivableAccount(Customer $customer, int $companyId): int
    {
     ...
     * @throws \InvalidArgumentException
     */
    private function syncAllocations(CustomerPayment $payment, array $allocationsData): void
    {
        $this->validateAllocations(
            $payment->company_id,
            $payment->customer_id,
