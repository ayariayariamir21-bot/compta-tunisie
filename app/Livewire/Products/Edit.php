...
class Edit extends Component
{
    ...
    public function mount(int $productId, CurrentCompany $currentCompany): void
    {
        ...
        $this->purchase_price = $product->purchase_price !== null ? (string) $product->purchase_price : null;
        $this->sale_price = $product->sale_price !== null ? (string) $product->sale_price : null;
        $this->tax_rate_id = $product->tax_rate_id;
        $this->sales_account_id = $product->sales_account_id;
        $this->purchase_account_id = $product->purchase_account_id;
    ...
    public function update(CurrentCompany $currentCompany, CurrentFiscalYear $currentFiscalYear, ProductService $productService): void
    {
        $company = $currentCompany->get(Auth::user());
        $fiscalYear = $currentFiscalYear->get(Auth::user());

        if (! $company || ! $this->product || $this->product->company_id !== $company->id) {
            abort(403);
        }
        ...
        try {
            $productService->updateProduct($this->product, $validated);
        ...
        session()->flash('success', "Le produit/service « {$this->name} » a été mis à jour.");
