...
class Create extends Component
{
    ...
    /**
     * Editable allocation rows per posted invoice of the selected customer.
     *
    ...
    /**
     * Preselect a customer and an invoice allocation when coming from an invoice.
     */
    public function loadInvoice(int $invoiceId): void
    {
        ...
        $remaining = app(CustomerPaymentService::class)->getInvoiceRemainingAmount($invoice);
        ...
        foreach ($this->allocations as $index => $row) {
            if ($row['invoice_id'] === (int) $invoice->id) {
    ...
    public function save(
        ...
        if (Auth::user()->cannot('create', [CustomerPayment::class, $company])) {
            abort(403);
        }
        ...
        session()->flash('success', "L'encaissement « {$payment->payment_number} » a été {$label}.");
        $this->redirect(route('customer-payments.show', $payment->id), navigate: true);
    }
    ...
    public function render(CurrentCompany $currentCompany): View
    {
        ...
        return view('livewire.customer-payments.create', [
            ...
            'journals' => $journals,
            'accounts' => $accounts,
            'statuses' => [CustomerPaymentStatus::DRAFT],
        ]);
    }
}
