...
class Create extends Component
{
    ...
    public function store(QuoteService $quoteService, CurrentCompany $currentCompany): void
    {
        ...
        if (Auth::user()->cannot('create', [Quote::class, $company])) {
            abort(403);
        }
        ...
        unset($validated['_key']);

        try {
            $quote = $quoteService->createDraft($validated);
        ...
        $label = $this->saveAndSend ? 'créé et envoyé' : 'créé';
        session()->flash('success', "Le devis « {$quote->quote_number} » a été {$label}.");
