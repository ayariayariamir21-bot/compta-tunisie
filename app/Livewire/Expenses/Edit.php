...
class Edit extends Component
{
    ...
    public function mount(int $expenseId): void
    {
        ...
        foreach ($expense->lines as $line) {
            $this->lines[] = [
                ...
                'label' => $line->label,
                'quantity' => $line->quantity !== null ? $service->toDecimal((string) $line->quantity) : '',
                'unit_price' => $service->toDecimal((string) $line->unit_price),
                'discount_percent' => $service->toDecimal((string) $line->discount_percent),
     ...
     */
    private function buildLinesData(): array
    {
        $data = [];
    ...
    public function save(
        ...
        try {
            $updated = $service->updateDraft($expense, [
        ...
        $label = $this->saveAndPost ? 'modifiée et comptabilisée' : 'modifiée';
        session()->flash('success', "La dépense « {$updated->expense_number} » a été {$label}.");
        $this->redirect(route('expenses.show', $updated->id), navigate: true);
    }
