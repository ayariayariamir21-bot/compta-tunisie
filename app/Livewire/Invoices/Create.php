...
class Create extends Component
{
    ...
    public function store(InvoiceService $invoiceService, CurrentCompany $currentCompany, CurrentFiscalYear $currentFiscalYear, CurrentAccountingPeriod $currentAccountingPeriod): void
    {
        ...
        if (Auth::user()->cannot('create', [Invoice::class, $company])) {
            abort(403);
        }
        ...
        try {
            $invoice = $invoiceService->createDraft($validated);

            if ($this->saveAndPost) {
                $invoiceService->post($invoice, (int) Auth::id());
            }
        ...
        session()->flash('success', "La facture « {$invoice->invoice_number} » a été {$label}.");
