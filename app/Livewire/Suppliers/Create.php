...
class Create extends Component
{
    ...
    public function generateCode(CurrentCompany $currentCompany, SupplierService $supplierService): void
    {
        ...
        $this->code = $supplierService->generateCode($company->id);
        $this->isGenerated = true;
    }

    public function store(CurrentCompany $currentCompany, CurrentFiscalYear $currentFiscalYear, SupplierService $supplierService): void
    {
        $company = $currentCompany->get(Auth::user());
        ...
        if (Auth::user()->cannot('create', [Supplier::class, $company])) {
            abort(403);
        }
        ...
        try {
            $supplierService->createSupplier($validated);
        ...
        session()->flash('success', "Le fournisseur « {$this->name} » a été créé.");
